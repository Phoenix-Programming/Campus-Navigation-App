...
# svg_to_graph.py: Inputs svg file
# outputs a json file of nodes with coordinates calculated from the svg file

# Usage: python svg_to_graph.py <svg_file>

# ensure that svg properties are set to display units and format units in cm
# install svgelements library with pip install svgelements first
...
def main() -> None:
	...
	try:
		...
		with open(filename, "w") as f:
			json.dump([node.to_dict() for node in nodes], f, indent=2) # convert each node in the dictionary to a dictionary format and save to json file
