...
export default function NodeConnectionsEditor() {
	...
	const removeConnection = () => {
		...
		setConnections((prev: Array<{ id: string; connections: string[] }>) => {
			//this is a function call
			return prev.map((conn: { id: string; connections: string[] }) => {
				// for all connections in edges
				if (conn.id === id1) {
					return {
						...conn,
	...
	const insertConnection = (id1: string, id2: string) =>
		setConnections((prev) => {
			const updated = structuredClone(prev); //copy edges
			...
			const addConnection = (from: string, to: string) => {
				...
				if (existing) {
					//if it exists
					if (!existing.connections.includes(to)) {
						// if it doesn't include the connection already
	...
	const forceConnection = (_id1: string, id2?: string) => {
		//WIP unused currently
		if (id2 !== null && id2 !== undefined) {
			console.log("Error, tried to force regular connection");
			return;
		} else {
	...
	const importConnections = (data: Array<{ id: string; connections: string[] }>) => {
		// data is array of node objects, with string id and array of connections
	...
	useGlobalKeydown((e: any) => {
		...
		if (
			e.pressedKeys.length === 1 &&
			(e.pressedKeys.includes("Control") || e.pressedKeys.includes("Alt") || e.pressedKeys.includes("shift"))
		) {
	...
	function handleChange(e: React.ChangeEvent<HTMLInputElement>) {
		...
		if (f.type === "application/json") {
			...
			f.text().then((text: string) => {
				...
				if (uplState === 2) {
					//if the svg file has already been uploaded, set the state to 3 to indicate that both files have been uploaded
					setUplState(3);
				} else if (uplState === 0) {
			...
			if (uplState === 1) {
				//if the json file has already been uploaded, set the state to 3 to indicate that both files have been uploaded
				setUplState(3);
			} else if (uplState === 0) {
...
function SVGViewer({
	...
	connections: Array<{ id: string; connections: string[] }>;
}) {
	...
	function onMouseDown(e: React.MouseEvent<HTMLDivElement>) {
		//event called to detect dragging for panning the image
	...
	function onMouseMove(e: React.MouseEvent<HTMLDivElement>) {
		//event for panning the image while dragging
...
function Nodes({
	...
	const nodesById = Object.fromEntries(json.map((node) => [node.id, node]));
	...
	const edges = connections.flatMap(
		(
			conn: { id: string; connections: string[] } //for each connection, create a edge for each connection inside it
		) =>
			conn.connections.map((targetId: string) => ({
				//targetId is each string inside the connections[] array inside the connection
				from: conn.id,
				to: targetId
			}))
	);
	...
	return (
		<div>
			...
			{edges.map((edge: { from: string; to: string }) => {
				//for each edge
...
function NodeOverlay({
	...
	forceConnection: (id: string, id2?: string) => void;
}) {
	...
	return (
		<div className="overlay">
			...
			{false &&
				selectedNode &&
				!secondSelectedNode && ( //Remove false to reactivate force button
					<>
						<br />
						{!forceHandle ? (
...
function ConnectionsOverlay({
	...
	edges: Array<{ id: string; connections: string[] }>;
	...
	return (
		<div className="connectionsOverlay">
			...
			<br></br>
			<label className="connectionsBtn">
				{" "}
				{/*Button to upload json of connections*/}
				Import existing Json
				<input type="file" accept=".json" onChange={handleChange} />
			</label>
			<br></br>
			<h1>{nConnections && <>{nConnections} connections loaded</>}</h1>
		</div>
	);
}
...
function AutoFill({
	...
	edges: Array<{ id: string; connections: string[] }>;
	json: any[];
	insertConnection: (id1: string, id2: string) => void;
}) {
			...
			if (node.type === "rm") {
				...
				hallway
					...
					.map((node: any, index: number) => {
						if (index < hallway.length - 1) {
							//if it's not the last node, connect it to the next node in the hallway
		...
		<>
			{!prompted && (
				<div className="autoFillOverlay">
					...
					as long as they are named properly <br></br>
					<br></br>
